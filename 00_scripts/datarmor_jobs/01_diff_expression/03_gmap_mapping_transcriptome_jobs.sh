...
for file in $(ls /scratch/home1/jleluyer/gamma/03_trimmed/*paired*.f*q.gz|sed 's/_R[12].paired.fastq.gz//'|sort -u)
do
...
#Submit jobs
for i in $(ls 00_scripts/datarmor_jobs/GSNAPTR_*sh)
do
qsub $i
done
